...
def test_raw_block_core_keeps_iouring_recovery_validation_serial(monkeypatch):
    spec = encode_object_key(make_object_key(51))
    core = object.__new__(RawBlockCore)
    core._lock = threading.Lock()
    core._index = {
        spec.encoded: type("Entry", (), {"offset": 4096, "size": 7})(),
    }
    core._lock_refcnt = {}
    core._meta_dirty_total = 0
    ...
    core.key_namespace = "object"
    core._read_slot_header = lambda offset: (spec.slot_identity, 7)
    ...
    assert list(core._index) == [spec.encoded]
